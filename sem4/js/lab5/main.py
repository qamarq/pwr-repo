    ...
    for (year, qty, freq), fpath in files.items():
        ...
        for dt_str, values in data["data"]:
            ...
            if mdate and args.start <= mdate <= args.end:
                matching_codes.update(
                    code for code, val in values.items() if val is not None
                )
...
def cmd_stats(args: argparse.Namespace) -> None:
    ...
    if not found_file:
        log.warning(
            "No files for quantity=%s frequency=%s", args.quantity, args.frequency
        )

    if not values:
        log.warning(
            "No measurements for station=%s in [%s, %s]",
            args.station,
            args.start,
            args.end,
        )
