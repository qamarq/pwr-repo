...
export const runFCFS: AlgorithmFunction = (initialHeadPosition, requests) => {
  ...
  while (allRequests.length > 0 || activeRequests.length > 0) {
    ...
    // W FCFS bierzemy pierwsze żądanie, które przybyło
    // Żądania są już posortowane wg arrivalTime, więc bierzemy pierwszy element
    const nextRequest = activeRequests.shift()!;

    // Oblicz koszt ruchu głowicy
